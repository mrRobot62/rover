...
class DriverControllerNode(Node):
    ...
    def __init__(self):
        ...
        #
        #
        # Bevor der ESP32Client gestartet werden kann muss sichergestellt sein, das der I2CNode
        # im Status 'active' ist
        #
        #
        # Bemerkung: die Publisher/Subscripts dürfen auch erst im Anschluß gesetzt werden, weil alles asynchron ist
        #           wird der Publisher vor dem esp32Client gestartet kommt es zu einem RunTime-Fehler, da direkt in den callback gesprungen wird
        #           und es noch kein esp32client-Objekt gibt
   
        #wait_lifecycle_node_active_async(self, 'i2c_node', self._after_wait_i2c_node)
        self._after_wait_i2c_node(True)
        self.get_logger().info('after wait_lifecycle_node_active_async()')


    def _after_wait_i2c_node(self, is_active: bool):
        try:
            #
        ...
        self.get_logger().info('DriverControllerNode ready.')
    ...
    def cmd_driver_callback(self, msg: Joy):
        ...
        if self.MODE == "MANUAL":
            ...
            if (now - self.last_i2c_time >= self.min_interval and
                ...
                self.get_logger().debug(f"WRITE_SERVO {steering:.5f} | {velocity:.5f}")
                if self.esp32client.is_available():          
                    self.esp32client.write_servo(
                         steering=steering,
                         velocity=velocity
                    )
                else:
                    self.get_logger().warn(f'[cmd_driver_callback] => self.esp32client not available, wait...')
